...
## ADR-031 — 큐가 스스로 비면 절전 방지를 풀고, 설정된 절전/종료를 실행한다

**배경.** 147개짜리 폴더는 밤새 돕니다. 사용자가 자리를 비우면 Windows가 유휴 타이머로 시스템을
절전 상태로 내려 큐가 멈췄고, 아침에 와 보면 절반만 끝나 있었습니다. 반대 방향의 요청도 함께
왔습니다 — "다 끝나면 알아서 꺼졌으면 좋겠다".

**결정.**

1. **큐가 실행 중인 동안 시스템 절전을 막는다.** `SetThreadExecutionState(ES_CONTINUOUS |
   ES_SYSTEM_REQUIRED)` 한 줄입니다. 디스플레이는 그대로 꺼지게 둡니다(`ES_DISPLAY_REQUIRED`
   없음) — 배치 작업에 모니터는 필요 없고, 켜 두면 번인과 전력만 낭비합니다. `JobQueueService`가
   `Running`을 알릴 때 걸고 `Idle`/`Paused`로 돌아올 때 풉니다.

2. **큐가 스스로 다 처리되면 설정된 동작(절전 / 최대 절전 / 시스템 종료)을 실행한다.**
   조건이 세 겹입니다:
   - **자연 종료만.** 직접 누른 중단이나 일시정지 뒤에는 실행하지 않습니다. 큐를 손으로 멈춘
     사용자는 PC가 꺼지기를 바란 것이 아닙니다. `JobQueueService`가 이 구분을 위해
     `QueueDrained` 이벤트를 새로 냅니다 — 실행 토큰이 취소되지 않았고 일시정지도 아닐 때만
     발생합니다.
   - **실제로 뭔가 처리했을 때만.** 완료 0건이면(예: 실행할 것이 없는데 시작을 누름) 실행하지
     않습니다.
   - **설정으로 고르는 안전판.** "실패하거나 취소된 작업이 없을 때만" 옵션(기본 켜짐)이 켜져
     있으면 실패·취소가 하나라도 있을 때 동작을 취소합니다 — 사용자가 무슨 일이 있었는지
     볼 수 있게 PC를 켜 둡니다.
   - 실행 직전 **30초 취소 카운트다운**이 항상 끼어듭니다.

**판단은 `PostQueueActionPolicy`(Domain, 순수)에 둡니다.** App은 `net10.0-windows`라 Linux
테스트에서 닿지 않으므로 §6.11 `ModelSelectionValidator`와 같은 분리입니다 — 규칙은 Domain에서
단위 테스트하고, 카운트다운 창과 `SetThreadExecutionState`/`SetSuspendState` P/Invoke만 App에
있습니다.

**시스템 종료는 `shutdown.exe /s /t 0`으로 셸 아웃합니다.** `ExitWindowsEx`는 프로세스가
`SeShutdownPrivilege`를 직접 켜야 하는데, 절전·최대 절전(`SetSuspendState`)은 권한 조정이
필요 없습니다. 굳이 한 API로 통일할 이유가 없습니다.

**설정에 새 키 둘.** `PostQueueAction`(enum, 기본 `None`), `PostQueueActionOnlyWhenAllSucceeded`
(bool, 기본 `true`). 성능 손잡이가 아니고 캐시 무효화와 무관하므로 §6.6 지문에는 넣지
않습니다.

**결과.**
- `QueueDrained` 이벤트가 생겼습니다. 지금 소비자는 큐 완료 후 동작 하나뿐이지만, "큐가 다
  끝났다"는 알림·소리 같은 후속 기능이 매달릴 자리이기도 합니다.
- **실기 미검증.** 절전 방지·절전·최대 절전·시스템 종료 전부 자동 테스트가 닿지 않습니다
  (P/Invoke, 실제 전원 상태). 검증되는 것은 `PostQueueActionPolicy`의 전 분기와 `QueueDrained`가
  중단·일시정지에 발생하지 않는다는 것까지입니다. 나머지는 사람의 몫입니다(ADR-020과 같은 이유).
- NVIDIA 이외 GPU·CPU 전용에서도 그대로 동작합니다 — 전원 API는 GPU와 무관합니다.
